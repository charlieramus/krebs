 ...
 * by then the cell has been sitting with NAD+ at zero and ATP decayed toward
 * denormal the whole time. Whether the pathway can restart from there is a
 * measurement rather than an argument, and this is the measurement.
 *
...
describe('recovery from a very long stall', () => {
  ...
  it('recovers through the payoff phase rather than through the preparatory one', () => {
    // The mechanism matters, not just the outcome. ATP is at denormal when the
    // unlock lands, so the preparatory phase cannot pay its 2 ATP entry cost and
    // cannot be what restarts the pathway. The stranded g3p is what lets the
    // payoff phase run first and make the ATP the preparatory phase then needs.
    // If a future balance change consumes g3p during the stall, recovery stops
    // being possible and this is the test that says so.
    const runtime = createAct1Runtime();
    let nowMs = 0;
    runtime.frame(nowMs);
    ...
    expect(runtime.snapshot.amounts[ATP] as number).toBeLessThan(1e-9);
  });
});
