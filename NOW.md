...
## What exists
    ...
    BRIEF.md          orientation, the idea and the reasoning
    PILLARS.md        scope contract
    PROGRESSION.md    four acts, unlock order, gating
    SIMULATION.md     engine spec, settled, awaiting implementation
    SAVE_SCHEMA.md    data contract, version 1, frozen
    SCIENCE.md        biological ground truth and citations
    DESIGN.md         visual contract, direction Honest Cartoon
    IDEAS.md          loose ideas, not a spec
    NOW.md            this file

    ECONOMY.md        not written, deliberate, needs a prototype first
    CONTENT_STYLE.md  not written, deliberate, written last
...
2. **Five timeline dates are unsourced.** ~4.0 Ga, ~2.7 Ga, ~2.0 to 1.5 Ga and ~1.6 Ga currently violate hard rule 1. A sourcing pass on SCIENCE.md is drafted and pending.
3. **Act 2 has no iron-sulfur target in the player's pathway.** PROGRESSION.md line 63 targets ROS damage at Fe-S enzymes, but no glycolytic enzyme has an Fe-S cluster and the TCA cycle does not unlock until act 3. Likely correct targets are pyruvate:ferredoxin oxidoreductase and ferredoxin. Needs a SCIENCE.md entry and a PROGRESSION.md correction. Part of the same pending sourcing pass.

## Open, not blocking

- **Working title is still TBD.** BRIEF.md line 4 says so and no naming shortlist exists. The wordmark is drawn as `krebs`, but the Krebs cycle unlocks roughly four hours in and does not exist during act 1.
...
- `STEADY_EPSILON` and `STEADY_WINDOW` are marked tune during prototype in SIMULATION.md Part 6.

## Next, in order

1. `git init` and commit everything as it stands.
2. Land the SCIENCE.md sourcing pass. Blocking items 2 and 3 both resolve here.
3. `/updatelog` for the vertical slice, in a fresh session.

## The vertical slice

Scope is fixed by BRIEF.md line 110 and should not grow: tick loop, one pool, glycolysis, the NAD+ constraint, fermentation, no UI polish.

SIMULATION.md already decides everything this needs, so the stages are implementation rather than design. In scope: fixed timestep accumulator, pools, Michaelis-Menten flux, two-phase update, negative pool proportional scaling, seeded PRNG, the conservation property test and the determinism test.

Out of scope for the slice: saves, offline progress, the design system, the timeline, the beast.

Order note: SIMULATION.md line 90 says the conservation test should exist before act 1 content does.

## Why the UI waits

The slice exists to answer two questions from BRIEF.md. Whether saturating kinetics feel like a game, and whether the NAD+ wall reads as interesting rather than annoying.
