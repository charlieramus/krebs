...
Amended on 2026-08-05 by an engineering review, which read the code rather than the document. It moved CI to the front, split the largest log in two, and priced four risks this plan did not carry. Those changes are in "What the engineering review changed" below. Its own outside voice found the sharpest item in the document and it is the act ordering, which was left unresolved on purpose with its price written down. **Resolved on 2026-08-20, five logs later and one handover after it became blocking: the order stands and the placeholder is oxygen at saturation.** See the risk row "Act 3 has no oxygen".
...
**The outside voice found the item this document is now least sure about.** Act 3 before act 2 may be incoherent rather than merely bold, because act 3's payoff needs oxygen and act 2 is what supplies it. It is written into the risk table with its price and was left open, because it is a product decision rather than an engineering one. **Taken on 2026-08-20 and the objection did not survive contact with the stoichiometry**: oxygen sets rate and not yield, so act 3's payoff figure never depended on act 2's schedule and only act 3's pacing did.
...
```
  ...
  V14  ACT 3, COMPLETE             the 15x payoff surface
                                   DECIDED 2026-08-20: act 3 needs oxygen
                                     and act 2 is what supplies it. Order
                                     stands, placeholder is saturation.
...
|---|---|---|
| **Act 3 has no oxygen** | Act 3's whole payoff is 2 to roughly 30 ATP, which needs oxygen as the terminal electron acceptor, and act 2 is what supplies it. `src/save/schema.ts` reserves `environment.oxygenLevel` and act 1 writes it as a literal 0 with the comment that this is not a placeholder. Building act 3 first means inventing a nonzero oxygen constant that act 2 must later take ownership of, and re-deriving act 3's balance when it does. It also inverts the thesis: docs/PROGRESSION.md says aerobic respiration is an opportunistic exploitation of a poison rather than the next rung, and act 3 without act 2 delivers exactly the ladder-climb this game refuses | **RESOLVED 2026-08-20. The order stands, in a hardened form.** The placeholder is oxygen at saturation rather than an invented level, so act 2 inherits "reach saturation by the boundary" and act 3 is not re-derived. Yield per glucose is stoichiometric and oxygen sets rate, so the 2 to 30 figure was never at risk and act 3's rates were. **The thesis inversion is answered by sequence rather than by argument**: acts are strictly sequential, the jump is a development door, and act 1's ending is a placeholder screen, so no player can reach act 3 until act 2 exists to be exploited. The ladder-climb is visible to a developer and to nobody else. See NOW.md "Settled 2026-08-20, the act ordering decision" |
...
- [x] **T6 (P1, human: ~0 / CC: ~0)** — decision — act 3 before act 2, priced. **TAKEN 2026-08-20, accepted in the hardened form.** The placeholder is oxygen at saturation, so the price is a constraint on act 2's schedule rather than a rebalance of act 3. Recorded in NOW.md "Settled 2026-08-20, the act ordering decision" and owned by UPDATELOGV14.md stage 1
  - Surfaced by: the outside voice. Act 3's payoff needs oxygen as terminal electron acceptor and act 2 supplies it
  - Files: this document, docs/PROGRESSION.md, NOW.md, UPDATELOGV14.md
...
**VERDICT:** CEO CLEARED. ENG CLEARED as of 2026-08-20. It read ENG NOT CLEARED for five logs on one open product decision, which gated the act 3 log rather than the three logs in front of it. Spine A, Spine B and the CI log were clear throughout and all three have shipped.

**RESOLVED DECISIONS:**
- ~~Act 3 before act 2.~~ **Accepted in writing on 2026-08-20, in a hardened form the review did not price.** Act 3's payoff is 2 to roughly 30 ATP and needs oxygen as the terminal electron acceptor, which act 2 supplies. The review's two exits were a placeholder oxygen constant with a DEPARTURE row and an act 3 rebalance, or flipping the order and withdrawing the value-ordering argument in Approach. **The first was taken and the rebalance was mostly bought off.** Yield per glucose is stoichiometric and oxygen sets rate rather than yield, so act 3's headline claim was never what the ordering put at risk. Setting the placeholder at saturation makes act 2's schedule inherit "reach saturation by the boundary", which is a target act 2 must hit rather than a number act 3 must chase. The value-ordering argument in Approach stands. See NOW.md "Settled 2026-08-20, the act ordering decision".
